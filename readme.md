...
```bash
git clone git@github.com:wagtail/collidersite.git
cd collidersite
vagrant up
vagrant ssh
...
installed, run:
    ...
    mkvirtualenv wagtailcollidersite
...
to help with this. It reads environment variables located in a file name `.env` in the top level directory of the project. The only variable we need to start is `DJANGO_SETTINGS_MODULE`:
    ...
    $ cp collidersite/settings/local.py.example collidersite/settings/local.py
    $ echo "DJANGO_SETTINGS_MODULE=collidersite.settings.local" > .env
...
[![Deploy](https://www.herokucdn.com/deploy/button.svg)](https://heroku.com/deploy?template=https://github.com/wagtail/collidersite)
...
the above), you can use the AWS CLI included with the requirements:
    ...
    heroku run aws s3 sync collidersite/media/original_images/ s3://<bucket-name>/original_images/
...
If you're a Python or Django developer, fork the repo and get stuck in! If you'd like to get involved you may find our [contributing guidelines](https://github.com/wagtail/collidersite/blob/master/contributing.md) a useful read.
...
`./manage.py dumpdata --natural-foreign --indent 2 -e auth.permission -e contenttypes -e wagtailcore.GroupCollectionPermission -e wagtailimages.filter -e wagtailcore.pagerevision -e wagtailimages.rendition  -e sessions > collidersite/base/fixtures/collidersite.json`
...
Make a pull request to https://github.com/wagtail/collidersite
